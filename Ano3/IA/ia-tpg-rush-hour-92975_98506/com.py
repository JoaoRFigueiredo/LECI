...
class Map:
    ...
    def find_indices(self, string, letter):
        indices = []
    ...
    def type_actions(self, piece: str):
        actions = []
        coord = self.piece_coordinates(piece)
        size = len(coord)
        
        x_min = min(c.x for c in coord)
        x_max = max(c.x for c in coord)
        y_min = min(c.y for c in coord)
        y_max = max(c.y for c in coord)

        if x_min == x_max:  # vertical
            try:
                if self.get(Coordinates(x_min, y_max + 1)) == 'o':
                    actions.append((piece, 's', size))
            except:
                pass
            try:
                if self.get(Coordinates(x_min, y_min - 1)) == 'o':
                    actions.append((piece, 'w', size))
            except:
                pass
        else:  # horizontal
            try:
                if self.get(Coordinates(x_min - 1, y_min)) == 'o':
                    actions.append((piece, 'a', size))
            except:
                pass
            try:
                if self.get(Coordinates(x_max + 1, y_min)) == 'o':
                    actions.append((piece, 'd', size))
            except:
                pass
