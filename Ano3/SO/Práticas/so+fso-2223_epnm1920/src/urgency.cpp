...
static pthread_mutex_t* patient_mutex; //acesso exclusivo para os pacientes
static pthread_cond_t* patient_cond; //variaveis de condição de cada paciente
...
void init_simulation(uint32_t np, uint32_t nn, uint32_t nd)
{
   ...
   //create arrays with their respective sizes (threads solutions)
   ...
   //initialize patient mutexes and condition variables
...
{
   mutex_lock(&patient_mutex[id]); //acesso exclusivo
...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   for (uint32_t i = 0; i<ndoctors; i++){
      pthread_create(&doctors_thrs[i], NULL, doctor_life, NULL);
   }

   //create patients' threads
   for (uint32_t i = 0; i<npatients; i++){
