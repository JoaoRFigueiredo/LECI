...
typedef struct
{
   ...
   pthread_mutex_t patient_access;
...
void init_simulation(uint32_t np)
{
   ...
   for (uint32_t i = 0; i<np; i++){
      cond_init(&hd->all_patients[i].patient_done, NULL);
      mutex_init(&hd->all_patients[i].patient_access, NULL);
...
void doctor_iteration()
{
   ...
   mutex_lock(&hd->all_patients[patient].patient_access);
   random_wait();
   ...
   mutex_unlock(&hd->all_patients[patient].patient_access);
}
...
{
   check_valid_name(hd->all_patients[id].name);
   mutex_lock(&hd->all_patients[id].patient_access );


   while(&hd->all_patients[id].done == 0){
      cond_wait(&hd->all_patients[id].patient_done, &hd->all_patients[id].patient_access);

   }
   
   printf("\e[30;01mPatient %s (number %u): health problems treated\e[0m\n", hd->all_patients[id].name, id);

   mutex_unlock(&hd->all_patients[id].patient_access);
}
