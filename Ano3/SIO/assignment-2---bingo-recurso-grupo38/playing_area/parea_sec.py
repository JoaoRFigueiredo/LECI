        ...
        for key, mask in events:
            ...
            if key.fileobj == sock:
                ...
                if data['header'] == 'register':
                    ...
                    if data['type'] == 'caller':
                        message = data
                        sign = data['signature'].encode('latin-1')
                        message.pop('signature')
                        #print(utils_sec.verify_signature(message, sign, data['key'], 'PKI'))
                        if  not utils_sec.verify_signature(message, sign, data['key'], 'PKI'):
                            print("Caller has provided an invalid signature regarding the register message")
                            unregister_and_close_client(selector, key.fileobj)
                            continue
                    ...
                    if data['type'] == 'player':
                        ...
                        #print(utils_sec.verify_signature(message, sign, data['key'], 'PKI'))
                        if  not utils_sec.verify_signature(message, sign, data['key'], 'PKI'):
                            print("Player has provided an invalid signature regarding the register message")
                            unregister_and_close_client(selector, key.fileobj)
                            continue
                    ...
                    for x in ids:
                        if(x != 0):
                            ...
                            if message['flag'] == True:
                                #deck invalido foi detetado
                                #game aborted
                                print("Invalid deck detected! Game aborted!")
                                sock.shutdown(socket.SHUT_RD)

                                # close the socket connection
                                sock.close()
                            #print(message['body'])
                    #did someone tamper with the deck
                    ...
                    for x in ids:
                        ...
                        if x != 0:
                            ...
                            if  message['flag']:
                                #print(message)
                                print("Players accused deck of being invalid")
                                print("Invalid deck detected! Game aborted!")
                                sock.shutdown(socket.SHUT_RD)
                                sock.close()
                            #     sys.exit()
