...
def challenge(client_sock, client_pbk, server_pbk, server_private_key):
    ...
    if answer['client_response'] == expected_response and time.time()-desafio['timestamp'] < tempoaceitavel :
...
def encrypt_deck(deck, symmkey):
    ...
    for x in deck:
        ...
        x = int.to_bytes(x, 2, 'little')
        x = iv + x

        cipher = Cipher(algorithms.AES(symmkey), modes.CFB(iv))
